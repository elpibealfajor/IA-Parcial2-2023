...
public class DFS<T>
{
    ...
    public List<T> Run(T start, Func<T, bool> isNodeSatisfactory, Func<T, List<T>> connections, int watchdog = 1000) //watchdog corta al analizar tantos nodos
    {
        Stack<T> pending = new Stack<T>(); //usamos queue en vez de lista porque saca el que ingresamos primero
